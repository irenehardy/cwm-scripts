...
class FlatRateDiscount
  def initialize(message, behaviour = :to_zero)
    @message = message
    @discount_applied = Money.zero
    @all_items = []
    @is_split = behaviour == :split
...
const campaigns = [{
  ...
  },
  {
    ...
    label: "Custom Bundle Discount",
    description: "Buy a certain combination of items for a flat rate price",
    inputs: {
      qualifer_behaviour: {
        type: "select",
        description: "Set the qualifier behaviour",
      ...
      bundle_price: {
        type: "number",
        description: "Flat price for all items in bundle"
      },
      bundle_items: {
        type: "objectArray",
        description: "Set the products that are part of a bundle",
